...
class GATWithEdgeFeatures(nn.Module):
    def __init__(self, in_channels, out_channels, edge_channels=3, heads=4):
        super().__init__()
    ...
    def forward(self, x, edge_index, edge_weight, edge_type, edge_distance):
        ...
        edge_contrib_aggregated = torch.zeros(num_nodes, self.gat.in_channels, device=x.device)
        ...
        out = self.gat(x_combined, edge_index)
...
class GATModel(BaseSTSCModel):
    def __init__(self, node_input_dim=30, esm_dim=640, hidden_dim=384,
                 output_dim=3, num_gnn_layers=5, dropout=0.1, heads=4):
        super().__init__(node_input_dim, esm_dim, hidden_dim, output_dim, 
                         num_gnn_layers, dropout, variant='gat')
        
        self.gnn = nn.ModuleList()
        for i in range(num_gnn_layers):
            in_channels = hidden_dim if i > 0 else hidden_dim
            self.gnn.append(GATWithEdgeFeatures(in_channels, hidden_dim, heads=heads))
