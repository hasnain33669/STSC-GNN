...
class GCNWithEdgeFeatures(nn.Module):
    def __init__(self, in_channels, out_channels, edge_channels=3):
        ...
        self.gcn = GCNConv(in_channels, out_channels, add_self_loops=True)
    ...
    def forward(self, x, edge_index, edge_weight, edge_type, edge_distance):
        ...
        edge_contrib_aggregated = torch.zeros(num_nodes, self.gcn.in_channels, device=x.device)
        ...
        out = self.gcn(x_combined, edge_index)
...
class GCNModel(BaseSTSCModel):
    def __init__(self, node_input_dim=30, esm_dim=640, hidden_dim=384,
        ...
        super().__init__(node_input_dim, esm_dim, hidden_dim, output_dim, 
                         num_gnn_layers, dropout, variant='gcn')
        
        self.gnn = nn.ModuleList()
        for i in range(num_gnn_layers):
            in_channels = hidden_dim if i > 0 else hidden_dim
            self.gnn.append(GCNWithEdgeFeatures(in_channels, hidden_dim))
